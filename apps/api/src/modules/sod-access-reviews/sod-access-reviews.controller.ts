...
export class SodAccessReviewsController {
  ...
  @Put("policies")
  @Permissions("settings:write")
  upsertPolicies(@Req() req: unknown, @Body() body: UpsertSodPoliciesDto) {
    return this.sod.upsertPolicies(req.user.companyId, body, req.user.sub);
  }

  @Get("violations")
  @Permissions("settings:write")
  listViolations(@Req() req: unknown, @Query() query: SodViolationsQueryDto) {
    return this.sod.listViolations(req.user.companyId, query.limit ?? 50);
  }

  @Get("summary")
  @Permissions("settings:write")
  summary(@Req() req: unknown) {
