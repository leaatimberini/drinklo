...
describe("AiCopilotService", () => {
  ...
  it("requires explicit approval before executing action", async () => {
    const service = new AiCopilotService(prisma as never, audit, ops);

    const response = await service.chat(userBase as never, "crear cupon del 10", "admin");
  ...
  it("enforces permissions on approve", async () => {
    ...
    await expect(service.approveProposal(user as never, "p1")).rejects.toBeInstanceOf(ForbiddenException);
  });

  it("generates immutable audit on approved execution", async () => {
    const service = new AiCopilotService(prisma as never, audit, ops);

    const result = await service.approveProposal(userBase as never, "p1", "ok");
  ...
  it("returns explain-and-cite references in chat responses", async () => {
    ...
    const response = await service.chat(userBase as never, "necesito ayuda con backups y deploy", "admin");
  ...
  it("enforces incident RAG scopes for non privileged users", async () => {
    ...
    const response = await service.chat(user as never, "incidente redis caido, sugeri runbook", "incident");
  ...
  it("creates incident action proposal and executes only after approval", async () => {
    ...
    const chat = await service.chat(user as never, "incidente redis timeout, sugeri runbook y mitigar", "incident");

    expect(chat.approvalRequired).toBe(true);
    expect(chat.proposals.some((p: unknown) => p.actionType === "RUN_INCIDENT_PLAYBOOK")).toBe(true);
    expect(prisma.coupon.create).not.toHaveBeenCalled();

    const approved = await service.approveProposal(user as never, "p_inc", "approved_incident_action");
